...
int main(void) {
  ...
  assert(!c.clutch_jog_active && c.clutch_pwm_percent==0);
  c=ready(); CarrierTick(&c,410,-6,0,'u',410,0);
  CarrierTick(&c,420,-6,0,'o',420,0);
  assert(c.state==CT_FAULT && c.error==CE_COMMAND && c.clutch_pwm_percent==0);
  c=ready(); CarrierTick(&c,410,-6,0,'u',410,0);
  CarrierTick(&c,420,-6,0,'k',420,0);
  assert(!c.clutch_jog_active && c.clutch_pwm_percent==0);
  c=ready(); CarrierTick(&c,410,-6,0,'x',410,0);
  assert(c.error==CE_COMMAND && c.pwm_percent==0 && c.clutch_pwm_percent==0);
  CarrierTick(&c,420,-6,0,'k',420,0);
