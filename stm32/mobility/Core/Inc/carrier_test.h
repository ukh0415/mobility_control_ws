...
/* Bench-only clutch-a control. Counts: (2404/3)*15*20/360=6010/9.
...
typedef struct {
  uint8_t state, error, referenced;
...
static int CarrierTargetForStep(const CarrierTest *c, int32_t step, int32_t *target)
{
  ...
  int64_t numerator = (int64_t)step * 6010;
                        ...
                        int32_t wheel, char cmd, uint32_t received_ms, int link_error)
{
  ...
  if (cmd != c->previous_command) {
    ...
    if (cmd == 'k') {
      ...
      if (c->state == CT_MOVING || previous != 'k' || now - c->quiet_since < CARRIER_SETTLE_MS) {
  ...
  if (llabs(error) <= CARRIER_COUNT_TOLERANCE) {
  ...
  /* Stop on overshoot instead of automatically reversing the mechanism. */
  if (error * direction < 0) { CarrierFault(c, CE_RANGE); return; }
